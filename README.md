...
# 별 헤는 밤
> 실시간 채팅 + AI 추천 및 자동 게시 기능이 통합된 커뮤니티 플랫폼  
> 구조적 문제 해결과 운영 환경을 고려한 백엔드 중심 설계 프로젝트

## 1. 프로젝트 개요
- **목표**: 인증/보안/세션/실시간 통신/자동화/AI 연동까지 포함한 실서비스 수준 백엔드 시스템 구축
- **역할**: 100% 개인 설계 및 구현 (백엔드 중심 + 프론트 일부 연동)
- **기간**: 2025.05 ~ 진행 중
- **기여 목적**: 기존 Outven(구조 설계) + Galaxy ML(이미지 분류 AI) 기반 확장형 프로젝트
...
| AI 연동 기능 |  
- 천체 이미지 자동 분류 모델 (Galaxy ML 기반, 예정)  
- 우주 관련 뉴스 자동 크롤링 → 게시판에 자동 등록 |
...
- 뉴스 크롤링 자동화: 지속적 콘텐츠 생산을 위한 운영 자동화 기능 적용
...
- Galaxy ML에서의 이미지 분류 모델 실험 경험을 향후 자동 태깅 기능으로 연결
- Python 크롤러를 통한 뉴스 자동 게시 기능 → **운영 자동화와 콘텐츠 지속성 확보**
