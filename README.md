...
이 프로젝트는 `.env` 파일과 `docker-compose.yml`을 통해  
DB, Redis, Spring Boot 애플리케이션을 컨테이너 기반으로 통합 실행할 수 있도록 구성되어 있습니다.

### 주요 연결 흐름

| 컴포넌트 | 호스트 이름 | 포트 | 연결 방식 |
|----------|--------------|------|------------|
| MySQL | `mysql` | 3306 | JDBC URL에서 `mysql`로 연결 (`docker-compose` 서비스명) |
| Redis | `redis` | 6379 | Spring Redis 설정에서 `redis`로 연결 |
| Spring Boot | `app` | 8080 | 내장 톰캣 실행 (`localhost:8080`) |

> `.env` 파일의 `DB_URL` 등 설정은 내부 컨테이너 DNS 이름을 기반으로 작성해야 하며,  
> `localhost`가 아닌 `mysql`, `redis` 등의 **서비스명**을 사용합니다.

---

### 📝 .env 파일 생성 방법

아래 내용을 복사해서 프로젝트 루트에 `.env` 파일로 저장하세요.  
`DB_PASSWORD` 등 민감한 항목은 **직접 비밀번호를 입력해서 사용**해야 합니다.

> 💡 `DB_PASSWORD`는 `docker-compose.yml`에 설정한 MySQL 비밀번호와 **반드시 동일**해야 합니다.

```env
# DB
DB_URL=jdbc:mysql://mysql:3306/byeolnight?serverTimezone=Asia/Seoul
DB_USERNAME=root
DB_PASSWORD=직접_입력해주세요  # 예: m1234

# MAIL
MAIL_USERNAME=직접_입력해주세요
MAIL_PASSWORD=직접_입력해주세요
MAIL_FROM=직접_입력해주세요
...
# COOLSMS
COOLSMS_API_KEY=직접_입력해주세요
COOLSMS_API_SECRET=직접_입력해주세요
COOLSMS_SENDER=직접_입력해주세요

# AWS
CLOUD_AWS_S3_BUCKET=직접_입력해주세요
CLOUD_AWS_ACCESS_KEY=직접_입력해주세요
CLOUD_AWS_SECRET_KEY=직접_입력해주세요
```
