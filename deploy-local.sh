#!/bin/bash
# 로컬 개발 환경 배포 스크립트
# 사용법: chmod +x deploy-local.sh && ./deploy-local.sh

set -e

echo "🚀 별 헤는 밤 로컬 개발 환경 배포 시작..."
...
docker compose -f docker-compose.local.yml up config-server -d
...
# 평문 값 가져오기 (로컬 환경용)
CONFIG_RESPONSE=$(curl -s -u config-admin:config-secret-2024 http://localhost:8888/byeolnight/local)
...
echo ""
echo "📋 로그 확인: docker logs -f byeolnight-local-app-1"