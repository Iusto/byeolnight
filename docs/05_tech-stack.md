...
- **시크릿 사전 검증**: 배포 스크립트에서 Config Server 복호화 결과·미해결 placeholder·최소 32바이트를 검사하고, 애플리케이션 시작 시에도 동일한 길이 조건을 재검증
...
- **Redis 7.0**:
  - 세션 저장소
  - 인증코드·JWT 블랙리스트 등 TTL 데이터 저장
  - 메일 메시지 큐 (List + 지연 Sorted Set)
  - 실시간 데이터 저장
  - 토큰 관리, 레이트리밋, 메시지 큐 처리

### 🔒 Redis 자료구조
**선택 배경**: 세션·토큰·레이트리밋과 비동기 메일 상태를 단일 인프라에서 원자적으로 관리
- **List**: 신규 메일 작업과 최종 실패 DLQ
- **Sorted Set**: 실행 시각을 score로 저장하는 지연 재시도 큐
- **Lua**: 실행 시각이 지난 메일 작업 조회와 제거를 원자적으로 처리
- **INCR/TTL**: IP 기반 Rate Limiting과 인증 시도 횟수 관리
- **메일 백오프**: 5/10/20/40초 간격으로 총 5회 시도 후 DLQ 이동
- **수용한 한계**: 신규 List 작업은 처리 확인이 없어 워커 종료 시 유실될 수 있지만, 재요청 가능한 인증 메일 특성과 현재 규모를 고려해 단순한 구조를 유지

### ☁️ AWS S3 (SDK v2.25.17) + CloudFront
**선택 배경**: SSRF 취약점 해결, 서버 부하 분산 및 글로벌 CDN으로 성능 향상
- **서버 검열 후 S3 업로드**: Vision 검열을 통과한 이미지 바이트만 공개 저장
...
- **예보**: 시간별/일별 예보
- **API 안정성**: 높은 가용성
- **장애 격리**: 1초 후 1회 재시도하고, 실패 시 마지막 성공 캐시를 `STALE`로 최대 2시간 제공
- **상태 공개**: `FRESH`/`STALE`/`UNAVAILABLE`과 마지막 성공 시각을 응답에 포함해 오래된 정보를 현재값처럼 표시하지 않음
...
- **메시지 큐**: Redis List + Sorted Set + Lua를 활용한 비동기 메일 전송 및 지연 재시도
- **레이트리밋**: TTL이 있는 원자적 카운터로 업로드·인증 요청 제한
- **의도적 분리**: 날씨 캐시는 단일 인스턴스 특성에 맞춰 Redis가 아닌 로컬 Caffeine을 사용
...
### ☁️ 왜 서버 검열 후 S3 업로드를 선택했나?
- **공개 전 검열**: Vision SafeSearch 통과 전에는 S3 공개 객체를 만들지 않음
- **실패 폐쇄**: 검열 API 오류도 업로드 거부로 처리
- **트레이드오프**: 최대 10MB 이미지 바이트를 WAS가 처리하지만 콘텐츠 노출 방지를 우선
