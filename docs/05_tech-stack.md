...
### 🗄️ Spring Data JPA + QueryDSL 5.0.0
...
- **MySQL 8.0**: 
  ...
  - 향상된 성능 및 보안
  - 복합 인덱스 최적화
  - 대용량 데이터 처리와 트랜잭션 안정성
- **Redis 7.0**:
  - 세션 저장소
  - 캐시 레이어
  - 메시지 큐 (Redisson RBlockingQueue)
  - 실시간 데이터 저장
  - 토큰 관리, 메시지 큐, 캐싱으로 성능 최적화

### 🔒 Redisson 3.37.0
**선택 배경**: 고급 Redis 기능 활용 - 메시지 큐, 원자적 연산, 분산 컬렉션
- **RBlockingQueue**: 비동기 메일 전송을 위한 메시지 큐 구현
- **RAtomicLong**: Rate Limiting을 위한 원자적 증가 연산 (INCR)
- **RMapCache**: TTL 지원 분산 캐시 맵
- **DLQ 패턴**: 실패 작업 추적 및 재시도 메커니즘
- **다양한 컬렉션**: Set, List, Map 등 분산 자료구조

### ☁️ AWS S3 (SDK v2.25.17) + CloudFront
...
### 🌐 Axios 1.7.9 + Native WebSocket/STOMP 7.1.1
...
- **Axios**:
  ...
  - 요청/응답 변환
  - 에러 처리
  - HTTP 요청 인터셉터와 JWT 자동 갱신
- **WebSocket/STOMP**:
  - 실시간 양방향 통신
  - 메시지 브로커 패턴
  - 자동 재연결
  - 하트비트
  - 순수 WebSocket 기반 실시간 통신
...
### 🎬 YouTube Data API
**선택 배경**: 우주 관련 영상 자동 수집 및 번역
- **영상 검색**: 키워드 기반 영상 검색
- **메타데이터**: 제목, 설명, 썸네일 등
- **통계 정보**: 조회수, 좋아요 등
- **채널 정보**: 채널별 영상 수집
- **API 안정성**: Google의 안정적인 API

### 🔐 OAuth2 소셜 로그인
**선택 배경**: 사용자 편의성과 보안성 향상
- **Google OAuth2**: 가장 널리 사용되는 소셜 로그인
- **Kakao OAuth2**: 한국 사용자 친화적
- **Naver OAuth2**: 한국 사용자 친화적
- **보안**: OAuth2 표준 프로토콜
- **사용자 편의**: 별도 회원가입 불필요
...
- **메시지 큐**: Redisson RBlockingQueue를 활용한 비동기 메일 전송 시스템
...
### ☁️ 왜 S3 Presigned URL을 선택했나?
- **서버 부하 분산**: 파일 업로드를 클라이언트에서 직접 처리
- **보안**: 임시 URL로 접근 권한 제어
- **확장성**: 대용량 파일 처리 시 서버 리소스 절약

### 🧪 왜 TestMockConfig를 도입했나?
- **코드 중복 제거**: 공통 Mock 설정으로 테스트 코드 93% 감소
- **테스트 안정성**: Lenient Mock으로 테스트 실행 안정성 향상
- **유지보수성**: 중앙화된 Mock 설정으로 변경 사항 일괄 적용

### 📦 왜 pnpm을 선택했나?
- **성능**: npm 대비 70% 빠른 설치 속도
- **디스크 효율**: 50% 적은 디스크 사용량
- **의존성 관리**: 더 엄격한 의존성 관리
- **호환성**: npm과 완전 호환
...
- [📊 데이터베이스 설계](./09_database-design.md)
