...
> **⚠️ 히트율 수치에 대한 정정**
>
> 이전 버전 문서는 단계별 히트율을 `0% → 20% → 60% → 90%(또는 95%)`로 기재했으나,
> **20%·60%·90%·95%는 계측 지표가 아니라 운영 로그를 보며 얻은 관찰치**였고
> 문서마다 값이 달랐다. 계측 근거가 있는 값은 아래 하나뿐이다.
>
> - **k6 부하테스트 + Actuator 카운터**: `hit 316,953 / miss 0`
> - **단, 이 시나리오는 사전 캐싱된 70개 도시만 조회**하므로 miss가 발생할 수 없다.
>   적중률 100%는 캐시 성능이 아니라 **시나리오가 만든 상한값**이다.
>
> 개선 자체는 실재하지만(키 공간 축소 → 공유율 상승 → 외부 호출 감소),
> **그 크기를 숫자로 주장할 근거는 현재 없다.** 실제 좌표 분포를 반영한 재측정이 필요하다.
...
### 결과
- 캐시 키 공간이 크게 줄어 동일 지역 요청 간 공유율 상승 (상승폭은 미계측)
- Redis 관리 부담 제거
...
- ⚠️ **히트율 상승폭은 미계측** — 위 "히트율 수치에 대한 정정" 참조
