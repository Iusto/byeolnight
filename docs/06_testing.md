...
miss 14건은 TLE 갱신 주기(12시간)와 패스 예측 캐시 TTL(2시간) 만료 시점에 발생한 정상적인 캐시 미스다. 캐시 미스 시 SGP4 궤도 계산(CPU-bound)이 실행되어 응답시간이 길어진다.

| 개선 방향 | 내용 |
|-----------|------|
| 임계값 현실화 | 99.995% 히트율 환경에서 miss 시 계산시간은 불가피 → p(95) 임계값을 60ms로 조정 |
| Proactive 계산 | 서버 시작 시 주요 도시 그리드를 미리 계산하여 최초 miss 제거 |
| 비동기 처리 | SGP4 계산을 `@Async`로 분리하여 응답 블로킹 최소화 |
