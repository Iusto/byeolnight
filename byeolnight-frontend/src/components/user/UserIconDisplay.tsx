...
const DefaultAsteroid = ({ className }: { className?: string }) => {
  ...
  if (AsteroidIcon) {
    // size를 주지 말고, wrapper(w/h) + w-full/h-full로만 통제
...
export default function UserIconDisplay({
                                          iconName,
                                          size = 'small',
                                          className = '',
                                        }: UserIconDisplayProps) {
  // iconName이 없으면 기본 소행성 아이콘 표시
  if (!iconName) {
    return (
  ...
  return (
      <div className={`icon-container ${getSizeClass(size)} rounded-lg ${className}`}>
        {/* size를 넘기지 말고 wrapper + w-full/h-full로만 크기 통제 */}
